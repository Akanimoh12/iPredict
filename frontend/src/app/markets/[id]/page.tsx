...
import { getStxBalance } from "@/services/stacks";
import { displaySTX, formatSTX, calculatePayout, truncateAddress } from "@/utils/helpers";
import {
  WIN_POINTS,
  LOSE_POINTS,
  WIN_TOKENS,
...
export default function MarketDetailPage({
  ...
  // Fetch native STX balance for betting
  useEffect(() => {
    if (!publicKey) {
      setStxBalance(0);
      return;
    ...
    let mounted = true;
    getStxBalance(publicKey).then((bal) => {
      if (mounted) setStxBalance(bal);
    });
          ...
          <ErrorBoundary fallbackTitle="Market info failed to load">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="card py-4 text-center">
                ...
                <p className="font-heading font-bold">{displaySTX(totalPool)}</p>
              </div>
              <div className="card py-4 text-center">
                <p className="text-xs text-slate-500 mb-1">YES Pool</p>
                <p className="font-heading font-bold text-accent-green">{displaySTX(market.totalYes)}</p>
              </div>
              <div className="card py-4 text-center">
                <p className="text-xs text-slate-500 mb-1">NO Pool</p>
                <p className="font-heading font-bold text-accent-red">{displaySTX(market.totalNo)}</p>
              </div>
              <div className="card py-4 text-center">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                  {winnerPayout > 0 && (
                    <div className="p-3 rounded-xl bg-surface-hover/50">
                      <span className="text-xs text-slate-500">STX Payout</span>
                      <p className="font-semibold text-accent-green">
                        {displaySTX(winnerPayout)}
                      </p>
                    </div>
                  )}
                  {market.cancelled && userBet && (
                    <div className="p-3 rounded-xl bg-surface-hover/50">
                      <span className="text-xs text-slate-500">STX Refund</span>
                      <p className="font-semibold text-accent-green">
                        {displaySTX(userBet.amount)}
                      </p>
                    </div>
                  )}
                  <div className="p-3 rounded-xl bg-surface-hover/50">
                  ...
                  <div key={`${evt.txHash}-${i}`} className="flex items-center gap-3 text-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <span className="text-slate-500">
                        {evt.type === "bet_placed"
                          ? `bet ${evt.amount ? formatSTX(BigInt(evt.amount)) : ""}`
          ...
          <ErrorBoundary fallbackTitle="Betting panel error">
            <div className="lg:sticky lg:top-24">
              <BettingPanel
                ...
                onSuccess={() => {
                  refetch();
                  // Refresh STX balance after bet
                  if (publicKey) {
                    getStxBalance(publicKey).then(setStxBalance);
                  }
                }}
              />
            </div>
