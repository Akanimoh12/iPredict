...
/**
 * Transaction lifecycle for Stacks claim:
 *   idle → building → signing (wallet popup) → submitting (polling API) → confirmed / failed
 *
 * The market service's `claim` uses `openContractCall` which opens the wallet
 * popup.  Its internal `onFinish` callback receives the `txId` and polls
 * `GET ${STACKS_API_URL}/extended/v1/tx/${txId}` until the status is
 * "success" or "abort_by_response".
 */
export type TxStage =
  | "idle"
  | "building"
  ...
  const submit = useCallback(
    async (marketId: number) => {
      ...
      try {
        ...
        // claim opens the wallet for approval, then polls the Stacks API
        // for tx confirmation via waitForTx (signing + submitting in one call)
        const txResult = await claim(publicKey, marketId);
