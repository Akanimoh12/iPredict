...
export default function ResolveMarketPanel() {
  ...
  const handleResolve = useCallback(
    async (marketId: number, outcome: boolean) => {
      ...
      try {
        const result = await resolveMarket(publicKey, marketId, outcome);
        if (result.success) {
  ...
  const handleCancel = useCallback(
    async (marketId: number) => {
      ...
      setTxState((prev) => ({ ...prev, [marketId]: { stage: "signing" } }));
      try {
        const result = await cancelMarket(publicKey, marketId);
        if (result.success) {
          setTxState((prev) => ({ ...prev, [marketId]: { stage: "confirmed" } }));
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-500">
                      ...
                      <span>
                        {((market.totalYes + market.totalNo) / 1e7).toFixed(1)} STX
                      </span>
                    </div>
                  </div>
