...
export default function BettingPanel({
  ...
  useEffect(() => {
    if (stage === "confirmed") {
      setShowShare(true);
      showToast(`Bet of ${amount} STX on ${side ? "YES" : "NO"} placed!`, "success");
      onSuccess?.();
  ...
  return (
    <div className="card space-y-5">
      ...
      {hasExistingBet && (
        <div className="px-4 py-3 rounded-xl bg-primary-600/10 border border-primary-600/20 text-sm">
          <span className="text-primary-400 font-medium">
            Your position: {userBet.amount.toFixed(2)} STX on{" "}
      ...
      <div className="space-y-2">
        <label className="text-xs text-slate-500 font-medium">Amount (STX)</label>
      ...
      {amount > 0 && (
        <div className="space-y-2 px-4 py-3 rounded-xl bg-surface/60 border border-surface-border">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-slate-400">{feeAmount.toFixed(2)} STX</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">If you win</span>
            <span className="text-accent-green font-medium">
              {payout.toFixed(2)} STX (+{profitPercent}%)
            </span>
          </div>
