...
let walletState = {
  publicKey: "ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5" as string | null,
  connected: true,
  connecting: false,
  error: null as null,
  connect: mockConnect,
...
describe("BettingPanel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    walletState = {
      publicKey: "ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5",
      connected: true,
      connecting: false,
      error: null,
      connect: mockConnect,
  ...
  it("shows balance", () => {
    ...
    expect(screen.getByText(/42\.50 STX/)).toBeInTheDocument();
  });
  ...
  it("displays existing position info when user has a bet", () => {
    ...
    expect(screen.getByText(/25\.00 STX on YES/)).toBeInTheDocument();
  });
  ...
  it("shows minimum bet validation", () => {
    ...
    expect(screen.getByText(/Minimum bet is 1 STX/)).toBeInTheDocument();
  });
